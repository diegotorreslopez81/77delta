...
for f in app/main.js app/api.js app/estado.js app/recargador.js app/rutas.js app/buscador.js app/shell.js app/ui.js app/tarjeta.js app/detalle.js app/dnd.js app/vistas/hoy.js app/vistas/objetivo.js app/vistas/tablero.js app/vistas/decisiones.js app/vistas/equipo.js app/vistas/expedientes.js app/tokens.css app/hq.css sw.js manifest.webmanifest v1/index.html v1/sw.js; do
  code=$(curl -s -o /dev/null -w '%{http_code}' "$B/$f"); [ "$code" = 200 ] || { echo "FALLO: $f -> $code"; exit 1; }; done
curl -fsS "$B/sw.js" | grep -q "hq-v14" || { echo "FALLO: sw.js no es la v14"; exit 1; }
